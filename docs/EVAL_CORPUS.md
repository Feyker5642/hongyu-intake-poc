...
| 路徑 | 嚴格比對 | 寬鬆比對* | FN 漏抓 | 備註 |
|---|---|---|---|---|
| 離線規則 | 87% | 89% | 138 | 0ms/筆 |
| LLM（gpt-5.5＋規則裁判） | 88% | **91%** | **76** | ~8.5s/筆，100/100 成功 |

*寬鬆＝同義異字互相包含即算同（「牛皮卡」vs「牛皮」、「有PDF」vs「PDF」）。

重點不在總分差 1–2 個百分點，在**漏抓近乎砍半**（138→76）：LLM 讀得懂規則
字典外的講法，而裁判層讓誤抓沒有失控（35→40）。防線在 LLM 路徑下依然成立：
提示注入 93%、大量缺漏 98% 兩類最高分。細目：`exports/llm_eval_results.json`。
